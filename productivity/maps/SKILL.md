...
- **maps_client.py can timeout** — the Python wrapper sometimes hangs on
  Nominatim and OSRM calls. If it times out (>30s), bypass it and call
  Overpass API directly via curl (see below).
- **Overpass API requires Content-Type header** — without it you get `406
  Not Acceptable`. Always use:
  ```bash
  curl -s --connect-timeout 10 --max-time 30 \
    -H "Content-Type: application/x-www-form-urlencoded" \
    -d 'data=[out:json][timeout:30];(...);out center body 50;' \
    "https://overpass-api.de/api/interpreter"
  ```
- **kumi.systems mirror** may return empty responses — stick with
  overpass-api.de as the primary endpoint.

## Fallback: Direct Overpass API via curl

When `maps_client.py` times out, use curl directly for POI discovery:

```bash
# Step 1: Get coordinates — use known lat/lon or estimate from address
# Example: 乌鲁木齐新市区昆明路158号 ≈ 43.852, 87.565

# Step 2: Query nearby POIs (adjust bbox: south,west,north,east)
curl -s --connect-timeout 10 --max-time 30 \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d 'data=[out:json][timeout:30];(
    node["amenity"~"cafe|restaurant|marketplace|fast_food"](43.84,87.54,43.87,87.59);
    node["leisure"~"park|garden|fitness_centre"](43.83,87.53,43.88,87.60);
    way["leisure"~"park|garden"](43.83,87.53,43.88,87.60);
    node["tourism"~"attraction|museum|viewpoint|artwork"](43.83,87.53,43.88,87.60);
  );out center body 50;' \
  "https://overpass-api.de/api/interpreter"

# Step 3: Calculate distances in Python (haversine)
# Walking speed ≈ 80m/min for flat terrain
```

See `references/travel-planning-with-overpass.md` for a complete
walkthrough including distance calculation and route formatting.
