...
description: 'Reference for the Claude API / Anthropic SDK — model ids, pricing, params,
  ...
  TRIGGER — read BEFORE opening the target file; don''t skip because it "looks like
  a one-liner" — whenever: the prompt names Claude/Anthropic in any form (Claude,
  Anthropic, Opus, Sonnet, Haiku, `anthropic`, `@anthropic-ai`, `claude-*`, `us.anthropic.*`,
  `[1m]`); the user asks about an LLM (pricing/model choice/limits/caching) — never
  answer from memory; OR the task is LLM-shaped with provider unstated (agent/MCP/tool-definition/multi-agent/RAG/LLM-judge/computer-use;
  ...
  over the project hits (run this grep FIRST if no provider named — don''t Read the
  file).'
version: 1.0.0
author: anthropics
...
metadata:
  hermes:
    ...
    - anthropics
    related_skills: []
    imported_from: anthropics/skills
