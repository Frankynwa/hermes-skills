...
**GUI/模拟器应用（LVGL、SDL 等）：**
```
✅ 编译通过 → open 打开应用 → 用户逐页点击验证 → 确认无卡死
❌ 编译通过 → "应该能用了" → 实际一点就卡死
```

**关键警示：** 对于 GUI 应用，`make` 成功 ≠ 功能正常。卡死（beachball）和崩溃（assertion crash）只能通过实际运行才能发现。如果用户说"打开给我检查"，必须 `open` 应用让用户逐项验证，而不是从终端判定。

**交叉引用验证（差距分析/审计场景）：** 当用户提供多份分析报告、差距清单或需求文档时，**必须逐条用 grep/search_files 交叉验证实际代码**后才能报告结论。禁止直接复制原始分析中的"缺失"标签而不核实——同一功能可能存在于其他文件中（如 config.c 中的面板被误标为"完全缺失"）。用户会明确指出这种错误。

**分析报告验证清单：**
```bash
# 对每项"缺失"声明，执行：
grep -rn "关键函数名" src/     # 确认是否在其他文件实现
wc -l src/ui_page_*.c           # 对比行数判断实现程度
grep -n "create_panel_" src/    # 列出所有已实现的子面板
```
