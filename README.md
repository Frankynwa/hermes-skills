...
- **`dispatching-parallel-agents`** — 当面对 2 个以上可以独立进行、无共享状态或顺序依赖的任务时使用
  `autonomous-ai-agents/dispatching-parallel-agents/`
...
- **`opencode-mimo`** — Configure and use OpenCode CLI with Xiaomi MiMo model. Use for delegating coding tasks to MiMo-powered OpenCode agent.
  `autonomous-ai-agents/opencode-mimo/`
...
- **`better-auth-best-practices`** — Configure Better Auth server and client, set up database adapters, manage sessions, add plugins, and handle environment variables. Use when users mention Better Auth, betterauth, auth.ts, or need to set up TypeScript authentication with email/pass...
  `best-practices/`
...
- **`pretext`** — Use when building creative browser demos with @chenglou/pretext — DOM-free text layout for ASCII art, typographic flow around obstacles, text-as-geometry games, kinetic typography, and text-powered generative art. Produces single-file HTML demos b...
  `creative/pretext/`
...
- **`code-security`** — Security guidelines for writing secure code. Use when writing code, reviewing code for vulnerabilities, or asking about secure coding practices like 'check for SQL injection' or 'review security'. IMPORTANT: Always consult this skill when writing ...
  `domain/code-security/`

- **`macos-network-audit`** — >
  `domain/macos-network-audit/`
...
- **`chinese-git-workflow`** — 国内 Git 平台配置参考——Gitee、Coding.net、极狐 GitLab、CNB 的 SSH/HTTPS/凭据/CI 接入差异与镜像同步配置。仅在用户显式 /chinese-git-workflow 时调用，不要根据上下文自动触发。
  `git-commit/chinese-git-workflow/`

- **`git-commit`** — Execute git commit with conventional commit message analysis, intelligent staging, and message generation. Use when user asks to commit changes, create a git commit, or mentions "/commit". Supports: (1) Auto-detecting type and scope from changes, ...
  `git-commit/`
...
- **`video-content`** — Video transcript extraction and summarization — YouTube, Douyin/TikTok, and other platforms. Covers transcript fetching, audio transcription via Whisper, and content transformation (summaries, threads, blogs, chapters).
...
- **`pptx`** — Use this skill any time a .pptx file is involved in any way — as input, output, or both. This includes: creating slide decks, pitch decks, or presentations; reading, parsing, or extracting text from any .pptx file (even if the extracted content wi...
  `pptx/`
...
- **`refactor`** — Surgical code refactoring to improve maintainability without changing behavior. Covers extracting functions, renaming variables, breaking down god functions, improving type safety, eliminating code smells, and applying design patterns. Less drasti...
  `refactor/`
...
- **`hermes-arxiv-agent-deploy`** — Deploy hermes-arxiv-agent for daily arXiv paper monitoring with Feishu delivery. Covers repo setup, cron job creation, and known operational pitfalls.
  `research/hermes-arxiv-agent-deploy/`
...
- **`offensive-osint`** — Operational arsenal for external red-team and bug-bounty reconnaissance.
  `research/offensive-osint/`

- **`osint-conflict-maps`** — >
  `research/osint-conflict-maps/`
...
- **`chinese-code-review`** — 中文 review 沟通参考——话术模板、分级标注（必须修复/建议修改/仅供参考）、国内团队常见反模式应对。仅在用户显式 /chinese-code-review 时调用，不要根据上下文自动触发。
  `software-development/chinese-code-review/`

- **`claude-api`** — Reference for the Claude API / Anthropic SDK — model ids, pricing, params,
  `software-development/claude-api/`

- **`composition-patterns`** — React composition patterns that scale. Use when refactoring components
  `software-development/composition-patterns/`
...
- **`react-native-skills`** — React Native and Expo best practices for building performant mobile apps.
  `software-development/react-native-skills/`

- **`react-view-transitions`** — Guide for implementing smooth, native-feeling animations using React's
  `software-development/react-view-transitions/`
...
- **`web-artifacts-builder`** — Suite of tools for creating elaborate, multi-component claude.ai HTML
  `software-development/web-artifacts-builder/`
...
- **`xlsx`** — Use this skill any time a spreadsheet file is the primary input or output. This means any task where the user wants to: open, read, edit, or fix an existing .xlsx, .xlsm, .csv, or .tsv file (e.g., adding columns, computing formulas, formatting, ch...
  `xlsx/`
