...
- **Desktop App:** Download Claude desktop app from https://anthropic.com — includes a Claude Code mode with full file editing, terminal, and codebase understanding in a graphical interface. The CLI (`claude`) and desktop app share the same auth session. **CRITICAL: The Desktop App authenticates via OAuth to Anthropic's official servers only.** It cannot be pointed to DeepSeek, OpenRouter, or any custom API endpoint. The `ANTHROPIC_BASE_URL` environment variable only works with the CLI. If you need DeepSeek/custom endpoints, use the CLI exclusively.
...
13. **Desktop App ≠ CLI for custom endpoints** — The Claude Desktop App authenticates via OAuth to Anthropic's official servers and cannot use DeepSeek, OpenRouter, or any custom `ANTHROPIC_BASE_URL`. Only the CLI (`claude -p`, `claude` REPL) supports custom API endpoints. The Desktop App's developer mode adds UI settings but does not unlock custom endpoint routing. If the user asks "can Claude Desktop App use DeepSeek?", the answer is no — don't guess or assume a proxy is working.
