...
**子智能体请求的 toolsets 可能被忽略。** 即使你在 `delegate_task` 中指定了 `toolsets: ["web", "search", "browser"]`，子智能体实际获得的工具可能只有 GitHub MCP（或其他受限子集）。这在以下场景中会导致连续失败：
- 搜索 Google Scholar、Semantic Scholar、DBLP 等学术数据库
- 访问大学官网查找教师信息
- 任何需要 HTTP GET 访问外部网页的研究任务

**症状：** 子智能体返回 "I do not have web browsing capabilities" 或只能做 GitHub 搜索。

**解决方案：**
1. **直接在主智能体中执行**：用 `terminal` + `curl` 访问外部 API（OpenAlex、Semantic Scholar、DBLP）
2. **用 `academic-data-enrichment` skill 的 curl 模式**：该 skill 已有 OpenAlex API 调用模板
3. **如果必须委派**：在 context 中明确告知子智能体用 `terminal` + `curl` 代替 web 工具

**经验法则：** 需要访问外部网站的研究任务，不要委派给子智能体——在主智能体中直接完成。
